...
class VoicePipeline:
    def __init__(self, llm, tts):
        self.ISSUE_COOLDOWN = 5      # 5 sec delay for same issue
        self.llm = llm
        self.tts = tts
    ...
    def process_event(self, event, exercise, metrics):
        ...
        if not self.last_issue:
            ...
            if issue == self.last_issue:
                if time.time() - self.last_spoken_at < self.ISSUE_COOLDOWN :
                    return None
            else:
                self.last_issue = issue
